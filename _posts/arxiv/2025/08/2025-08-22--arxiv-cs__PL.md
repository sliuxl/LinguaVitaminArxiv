---
title: "cs.PL @ 2025-08-22: 033"
date: 2025-08-22
layout: post
---

- [00](#article-0) | **08-21 (4)** | Active Learning for Neurosymbolic Program Synthesis | Aktives Lernen für neurosymbolische Programmsynthese | 活动学习用于神经回旋程序合成 | [2508.15750v1](http://arxiv.org/abs/2508.15750v1)
- [01](#article-1) | 08-21 | Structural Temporal Logic for Mechanized Program Verification | Strukturelle Temporale Logik für mechanisierte Programmverifizierung | 机械化方案核查结构时时逻辑 | [2410.14906v7](http://arxiv.org/abs/2410.14906v7)
- [02](#article-2) | 08-21 | React-tRace: A Semantics for Understanding React Hooks | React-tRace: Eine Semantik zum Verständnis von React Hooks | React-trace:理解反应钩的语义 | [2507.05234v2](http://arxiv.org/abs/2507.05234v2)
- [03](#article-3) | 08-21 | Compositional Symbolic Execution for the Next 700 Memory Models   (Extended Version) | Kompositionssymbolische Ausführung für die nächsten 700 Speichermodelle (erweiterte Version) | 未来700个内存模型(扩展版本)的构成符号执行 | [2508.15576v1](http://arxiv.org/abs/2508.15576v1)
- [04](#article-4) | 08-21 | Correct Black-Box Monitors for Distributed Deadlock Detection:   Formalisation and Implementation (Technical Report) | Korrekte Black-Box-Monitore zur Distributed Deadlock Detection: Formalisierung und Implementierung (Technischer Bericht) | 用于分布式死锁检测的正确黑盒监视器:正规化和实施(技术报告) | [2508.14851v2](http://arxiv.org/abs/2508.14851v2)
- [05](#article-5) | 08-21 | Mini-Batch Robustness Verification of Deep Neural Networks | Mini-Batch Robustheitsprüfung von tiefen neuralen Netzwerken | 深神经网络微型批量强力核查 | [2508.15454v1](http://arxiv.org/abs/2508.15454v1)
- [06](#article-6) | 08-21 | Fair Termination for Resource-Aware Active Objects | Faire Beendigung für ressourcenschonende aktive Objekte | 资源软件活动对象的公平终止 | [2508.15333v1](http://arxiv.org/abs/2508.15333v1)
- [07](#article-7) | 08-21 | Exploring the Theory and Practice of Concurrency in the   Entity-Component-System Pattern | Erforschung der Theorie und Praxis der Übereinstimmung im Entity-Component-System-Muster | 探讨实体-构成部分-系统模式中的货币统一理论和实践 | [2508.15264v1](http://arxiv.org/abs/2508.15264v1)
- [08](#article-8) | 08-21 | Encode the $\forall\exists$ Relational Hoare Logic into Standard Hoare   Logic | Kodieren Sie die $\forall\exists$ Relational Hoare Logic in Standard Hoare Logic | 将 $\ forall\ diverses $lational Hoare 逻辑编码为标准 Hoare 逻辑 | [2504.17444v2](http://arxiv.org/abs/2504.17444v2)
- [09](#article-9) | 08-21 | Probabilistic Inference for Datalog with Correlated Inputs | Probabilistische Schlussfolgerung für Datalog mit korrelierten Eingängen | 带有相关输入量的数据图表概率推断 | [2508.15166v1](http://arxiv.org/abs/2508.15166v1)
- [10](#article-10) | 08-21 | Big-Stop Semantics: A Simple Way to Get the Benefits of Small-Step   Semantics in a Big-Step Judgment | Big-Stop Semantics: Ein einfacher Weg, um die Vorteile von Small-Step Semantics in einem Big-Step Urteil zu bekommen | 大站式语义学:在大站式判决中 获得小型语义学好处的简单方法 | [2508.15157v1](http://arxiv.org/abs/2508.15157v1)
- [11](#article-11) | 08-21 | Software Model Checking via Summary-Guided Search (Extended Version) | Software-Modellprüfung über Zusammenfassungs-geführte Suche (erweiterte Version) | 通过简易辅助搜索(扩展版本)核对软件模型 | [2508.15137v1](http://arxiv.org/abs/2508.15137v1)
- [12](#article-12) | **08-20 (3)** | Homomorphism Calculus for User-Defined Aggregations | Homomorphismus Calculus für benutzerdefinierte Aggregationen | 用户定义聚合的自定义聚合计算 | [2508.15109v1](http://arxiv.org/abs/2508.15109v1)
- [13](#article-13) | 08-20 | Choreographic Quick Changes: First-Class Location (Set) Polymorphism | Choreographische schnelle Änderungen: Standort der ersten Klasse (Set) Polymorphismus | 舞蹈快速变化:第一类位置(Set)多形态 | [2506.10913v2](http://arxiv.org/abs/2506.10913v2)
- [14](#article-14) | 08-20 | Reasoning about Weak Isolation Levels in Separation Logic | Begründung schwacher Isolationsebenen in der Trennungslogik | 分离逻辑中薄弱隔离水平的原因 | [2501.14421v2](http://arxiv.org/abs/2501.14421v2)
- [15](#article-15) | 08-20 | Gradual Metaprogramming | Stufenweise Metaprogrammierung | 渐进元元方案 | [2506.09043v3](http://arxiv.org/abs/2506.09043v3)
- [16](#article-16) | 08-20 | Proto-Quipper with Reversing and Control | Proto-Quipper mit Reversing und Kontrolle | 具有校正和控制功能的预置置置信器 | [2410.22261v2](http://arxiv.org/abs/2410.22261v2)
- [17](#article-17) | 08-20 | Reachability is Decidable for ATM-Typable Finitary PCF with Effect   Handlers | Erreichbarkeit ist für ATM-Typable Finitary PCF mit Effekt-Handlern entscheidend | 对于带有效果操作器的自动取款机易失能性低调可打字软性PCF | [2508.12572v2](http://arxiv.org/abs/2508.12572v2)
- [18](#article-18) | 08-20 | Close is Good Enough: Component-Based Synthesis Modulo Logical   Similarity | Schließen ist gut genug: Komponentenbasierte Synthese Modulo Logische Ähnlichkeit | 关闭为良好 足够 : 基于组件的合成模型逻辑相似性 | [2508.14614v1](http://arxiv.org/abs/2508.14614v1)
- [19](#article-19) | 08-20 | Tuning Random Generators: Property-Based Testing as Probabilistic   Programming | Tuning Random Generators: Eigenschaftsbasierte Prüfung als probabilistische Programmierung | 测算随机发电机:以财产为基础的测试作为概率规划 | [2508.14394v1](http://arxiv.org/abs/2508.14394v1)
- [20](#article-20) | **08-19 (2)** | Prompt Orchestration Markup Language | Prompt Orchestrierung Markup Sprache | 提示管管标记语言 | [2508.13948v1](http://arxiv.org/abs/2508.13948v1)
- [21](#article-21) | 08-19 | Bisimilarity and Simulatability of Processes Parameterized by Join   Interactions | Bisimilarität und Simulatierbarkeit von Prozessen, die durch Join Interactions parametriert werden | 联合互动所显示的进程参数的双相性和可模拟性 | [2508.13611v1](http://arxiv.org/abs/2508.13611v1)
- [22](#article-22) | 08-19 | Reactive Semantics for User Interface Description Languages | Reaktive Semantik für Benutzeroberfläche Beschreibung Sprachen | 用户界面描述语言的自反应语义 | [2508.13610v1](http://arxiv.org/abs/2508.13610v1)
- [23](#article-23) | **08-18 (1)** | Strengthening Programming Comprehension in Large Language Models through   Code Generation | Stärkung des Programmierverständnisses in großen Sprachmodellen durch Codegenerierung | 通过代码生成加强大语言模式的方案编制理解 | [2508.12620v1](http://arxiv.org/abs/2508.12620v1)
- [24](#article-24) | **08-17 (7)** | Type-Driven Prompt Programming: From Typed Interfaces to a Calculus of   Constraints | Typgetriebene Prompt-Programmierung: Von eingegebenen Schnittstellen zu einem Calculus of Constraints | 类型驱动快速编程: 从键入接口到制约的计算 | [2508.12475v1](http://arxiv.org/abs/2508.12475v1)
- [25](#article-25) | 08-17 | Controlling Copatterns: There and Back Again (Extended Version) | Kopatterns steuern: Es und wieder zurück (Erweiterte Version) | 控制集散器: 重置和倒置( 扩展版本) | [2508.12427v1](http://arxiv.org/abs/2508.12427v1)
- [26](#article-26) | **08-16 (6)** | Certified Compilation based on Gödel Numbers | Zertifizierte Zusammenstellung basierend auf Gödel-Nummern | 基于哥德尔号的核证汇编 | [2508.12054v1](http://arxiv.org/abs/2508.12054v1)
- [27](#article-27) | 08-16 | Discovering Expert-Level Nash Equilibrium Algorithms with Large Language   Models | Entdeckung von Experten-Nash-Equilibrium-Algorithmen mit großen Sprachmodellen | 发现具有大语言模型的专家级 Nash 平衡等级与大语言模型 | [2508.11874v1](http://arxiv.org/abs/2508.11874v1)
- [28](#article-28) | **08-15 (5)** | CoMoNM: A Cost Modeling Framework for Compute-Near-Memory Systems | CoMoNM: Ein Kostenmodellierungsrahmen für Compute-Near-Memory-Systeme | CONMNM: 计算近记忆系统的成本建模框架 | [2508.11451v1](http://arxiv.org/abs/2508.11451v1)
- [29](#article-29) | 08-15 | Towards Efficient Hash Maps in Functional Array Languages | Auf dem Weg zu effizienten Hash-Karten in funktionalen Array-Sprachen | 努力在功能阵列语言中建立高效的散列地图 | [2508.11443v1](http://arxiv.org/abs/2508.11443v1)
- [30](#article-30) | 08-15 | Generic Reduction-Based Interpreters (Extended Version) | Generische, reduktionsbasierte Interpreten (erweiterte Version) | 以一般裁减为基础的口译员(扩大版本) | [2508.11297v1](http://arxiv.org/abs/2508.11297v1)
- [31](#article-31) | **08-14 (4)** | Generating Compilers for Qubit Mapping and Routing | Kompiler für Qubit Mapping und Routing generieren | 正在生成Qubit 绘图和运行运行的编译器 | [2508.10781v1](http://arxiv.org/abs/2508.10781v1)
- [32](#article-32) | 08-14 | A bargain for mergesorts -- How to prove your mergesort correct and   stable, almost for free | Ein Schnäppchen für Mergesorts -- Wie Sie Ihr Mergesort korrekt und stabil beweisen, fast kostenlos | 合并的讨价还价 -- -- 如何证明合并的正确和稳定,几乎是免费的 | [2403.08173v3](http://arxiv.org/abs/2403.08173v3)

## Article 0
### Title@2025-08-21 (4): Active Learning for Neurosymbolic Program Synthesis

**Title**: Active Learning for Neurosymbolic Program Synthesis | Aktives Lernen für neurosymbolische Programmsynthese | 活动学习用于神经回旋程序合成 [2508.15750v1](http://arxiv.org/abs/2508.15750v1)

**Authors** (5): Celeste Barnaby, Qiaochu Chen, Ramya Ramalingam, Osbert Bastani, Isil Dillig

The goal of active learning for program synthesis is to synthesize the desired program by asking targeted questions that minimize user interaction. While prior work has explored active learning in the purely symbolic setting, such techniques are inadequate for the increasingly popular paradigm of neurosymbolic program synthesis, where the synthesized program incorporates neural components. When applied to the neurosymbolic setting, such techniques can -- and, in practice, do -- return an unintended program due to mispredictions of neural components. This paper proposes a new active learning technique that can handle the unique challenges posed by neural network mispredictions. Our approach is based upon a new evaluation strategy called constrained conformal evaluation (CCE), which accounts for neural mispredictions while taking into account user-provided feedback. Our proposed method iteratively makes CCE more precise until all remaining programs are guaranteed to be observationally equivalent. We have implemented this method in a tool called SmartLabel and experimentally evaluated it on three neurosymbolic domains. Our results demonstrate that SmartLabel identifies the ground truth program for 98% of the benchmarks, requiring under 5 rounds of user interaction on average. In contrast, prior techniques for active learning are only able to converge to the ground truth program for at most 65% of the benchmarks.

积极学习用于程序合成的目的是通过提出尽量减少用户互动的针对性问题来综合理想的方案。 虽然先前的工作在纯粹象征性的环境下探索了积极学习, 但这些技术不足以适应日益流行的神经同步方案合成模式, 即综合程序包含神经元组成部分。 当应用到神经同步环境时, 这些技术可以 -- -- 实际上,可以 -- -- 并且实际上 -- -- 返回一个因神经元组成部分的误解而导致的意外程序。 本文提议一种新的积极学习技术, 能够处理神经网络错误预测所构成的独特挑战。 我们的方法基于一种新的评价战略, 即限制符合性评价(CCEE), 即考虑到用户提供的反馈, 记录神经系统错误的新的评价战略。 我们提议的方法反复使CECE更加精确, 直到所有剩余方案都保证观测到等同。 我们已在一个名为SmartLabel的工具中应用了这种方法, 并在三个神经同步域进行实验性评价。 我们的结果表明, SmartLabel 确定了98 % 基准的地面真相方案, 平均需要低于5轮用户互动。 相比之下, 之前的精确学习程序的基础技术只有65 基本标准。

---

## Article 1
### Title@2025-08-21 (4): Structural Temporal Logic for Mechanized Program Verification

**Title**: Structural Temporal Logic for Mechanized Program Verification | Strukturelle Temporale Logik für mechanisierte Programmverifizierung | 机械化方案核查结构时时逻辑 [2410.14906v7](http://arxiv.org/abs/2410.14906v7)

**Authors** (4): Eleftherios Ioannidis, Yannick Zakowski, Steve Zdancewic, Sebastian Angel

Mechanized verification of liveness properties for infinite programs with effects and nondeterminism is challenging. Existing temporal reasoning frameworks operate at the level of models such as traces and automata. Reasoning happens at a very low-level, requiring complex nested (co-)inductive proof techniques and familiarity with proof assistant mechanics (e.g., the guardedness checker). Further, reasoning at the level of models instead of program constructs creates a verification gap that loses the benefits of modularity and composition enjoyed by structural program logics such as Hoare Logic. To address this verification gap, and the lack of compositional proof techniques for temporal specifications, we propose Ticl, a new structural temporal logic. Using ticl, we encode complex (co-)inductive proof techniques as structural lemmas and focus our reasoning on variants and invariants. We show that it is possible to perform compositional proofs of general temporal properties in a proof assistant, while working at a high level of abstraction. We demonstrate the benefits of Ticl by giving mechanized proofs of safety and liveness properties for programs with scheduling, concurrent shared memory, and distributed consensus, demonstrating a low proof-to-code ratio.

现有时间推理框架在轨迹和自成一体等模型层面运作; 原因发生于非常低的水平,需要复杂的嵌套(共同)感化验证技术和熟悉校对助理机械师(例如守衛檢查器) 。此外, 模型层面的推理而不是程序构造造成了一种核查差距,从而丧失了Hoare Logic等结构性程序逻辑所享受的模块化和构成的效益。 为了解决这一核查差距,以及缺乏时间规格的构成验证技术,我们提议采用新的结构性时间逻辑Ticl。我们用Ticl,将复合(共同)感化验证技术编码成结构性灵质,将我们的推理集中在变异和变异性上。我们表明,有可能在证据助理中进行一般时间属性的构成证明,同时在高度抽象的状态下工作。我们通过为有排期、并行的记忆率和分布式的系统提供安全性和活性特性的机械化证明来证明。

---

## Article 2
### Title@2025-08-21 (4): React-tRace: A Semantics for Understanding React Hooks

**Title**: React-tRace: A Semantics for Understanding React Hooks | React-tRace: Eine Semantik zum Verständnis von React Hooks | React-trace:理解反应钩的语义 [2507.05234v2](http://arxiv.org/abs/2507.05234v2)

**Authors** (3): Jay Lee, Joongwon Ahn, Kwangkeun Yi

React has become the most widely used web front-end framework, enabling the creation of user interfaces in a declarative and compositional manner. Hooks are a set of APIs that manage side effects in function components in React. However, their semantics are often seen as opaque to developers, leading to UI bugs. We introduce React-tRace, a formalization of the semantics of the essence of React Hooks, providing a semantics that clarifies their behavior. We demonstrate that our model captures the behavior of React, by theoretically showing that it embodies essential properties of Hooks and empirically comparing our React-tRace-definitional interpreter against a test suite. Furthermore, we showcase a practical visualization tool based on the formalization to demonstrate how developers can better understand the semantics of Hooks.

重新反应已成为最广泛使用的网络前端框架, 使得能够以声明和组成方式创建用户界面。 钩子是一套控制反应功能组件副作用的API。 但是, 它们的语义对于开发者来说常常被视为不透明, 导致 UI 错误。 我们引入了 React- trace, 将 React- trace 精髓的语义正规化, 提供了一种能澄清其行为的语义。 我们证明我们的模型能够捕捉回行为, 从理论上表明它体现了 Hooks 的基本特性, 并从经验上比较了我们的 React- trace- 定义解释器与测试套件。 此外, 我们展示了一个基于正规化的实用的可视化工具, 以显示开发者如何更好地理解 Hooks 的语义 。

---

## Article 3
### Title@2025-08-21 (4): Compositional Symbolic Execution for the Next 700 Memory Models   (Extended Version)

**Title**: Compositional Symbolic Execution for the Next 700 Memory Models   (Extended Version) | Kompositionssymbolische Ausführung für die nächsten 700 Speichermodelle (erweiterte Version) | 未来700个内存模型(扩展版本)的构成符号执行 [2508.15576v1](http://arxiv.org/abs/2508.15576v1)

**Authors** (6): Andreas Lööw, Seung Hoon Park, Daniele Nantes-Sobrinho, Sacha-Élie Ayoun, Opale Sjöstedt, Philippa Gardner

Multiple successful compositional symbolic execution (CSE) tools and platforms exploit separation logic (SL) for compositional verification and/or incorrectness separation logic (ISL) for compositional bug-finding, including VeriFast, Viper, Gillian, CN, and Infer-Pulse. Previous work on the Gillian platform, the only CSE platform that is parametric on the memory model, meaning that it can be instantiated to different memory models, suggests that the ability to use custom memory models allows for more flexibility in supporting analysis of a wide range of programming languages, for implementing custom automation, and for improving performance. However, the literature lacks a satisfactory formal foundation for memory-model-parametric CSE platforms.   In this paper, inspired by Gillian, we provide a new formal foundation for memory-model-parametric CSE platforms. Our foundation advances the state of the art in four ways. First, we mechanise our foundation (in the interactive theorem prover Rocq). Second, we validate our foundation by instantiating it to a broad range of memory models, including models for C and CHERI. Third, whereas previous memory-model-parametric work has only covered SL analyses, we cover both SL and ISL analyses. Fourth, our foundation is based on standard definitions of SL and ISL (including definitions of function specification validity, to ensure sound interoperation with other tools and platforms also based on standard definitions).

多个成功的构成符号执行(CSE)工具和平台利用分离逻辑(SL)进行组成核查和(或)不正确的分离逻辑(ISL),用于组成错误调查,包括Verifast、Viper、Gillian、CN和Infer-Pulse。Gillian平台是记忆模型的唯一参数化的CSE平台,这是记忆模型中唯一一个能够即时到不同记忆模型的CSE平台,这意味着它可以即时到不同的记忆模型,表明使用自定义记忆模型的能力允许在支持分析范围广泛的编程语言、实施定制自动化和改进性能方面有更大的灵活性。然而,文献缺乏一个令人满意的形式基础,用于记忆模型参数化 CSEE平台。在本文件中,在Gillian的启发下,我们为记忆模型模型参数化 CSESE平台提供了新的正式基础。我们的基础以四种方式推进了艺术状态。 首先,我们机械化了我们的基础(在互动的词质证中) 。第二,我们通过将它恢复到广泛的记忆模型模型模型中,包括C和CHEIRI的模型模型模型模型。第三,我们基于以前的SL标准级定义的SL定义和SL基础分析,也包括了我们先前的SL标准基础的SL定义。

---

## Article 4
### Title@2025-08-21 (4): Correct Black-Box Monitors for Distributed Deadlock Detection:   Formalisation and Implementation (Technical Report)

**Title**: Correct Black-Box Monitors for Distributed Deadlock Detection:   Formalisation and Implementation (Technical Report) | Korrekte Black-Box-Monitore zur Distributed Deadlock Detection: Formalisierung und Implementierung (Technischer Bericht) | 用于分布式死锁检测的正确黑盒监视器:正规化和实施(技术报告) [2508.14851v2](http://arxiv.org/abs/2508.14851v2)

**Authors** (3): Radosław Jan Rowicki, Adrian Francalanza, Alceste Scalas

Many software applications rely on concurrent and distributed (micro)services that interact via message-passing and various forms of remote procedure calls (RPC). As these systems organically evolve and grow in scale and complexity, the risk of introducing deadlocks increases and their impact may worsen: even if only a few services deadlock, many other services may block while awaiting responses from the deadlocked ones. As a result, the "core" of the deadlock can be obfuscated by its consequences on the rest of the system, and diagnosing and fixing the problem can be challenging.   In this work we tackle the challenge by proposing distributed black-box monitors that are deployed alongside each service and detect deadlocks by only observing the incoming and outgoing messages, and exchanging probes with other monitors. We present a formal model that captures popular RPC-based application styles (e.g., gen_servers in Erlang/OTP), and a distributed black-box monitoring algorithm that we prove sound and complete (i.e., identifies deadlocked services with neither false positives nor false negatives). We implement our results in a tool called DDMon for the monitoring of Erlang/OTP applications, and we evaluate its performance.   This is the first work that formalises, proves the correctness, and implements distributed black-box monitors for deadlock detection. Our results are mechanised in Coq. DDMon is the companion artifact of this paper.

许多软件应用程序依靠同时和分布的(微观)服务,通过信息传递和各种形式的远程程序电话(RPC)进行互动。 随着这些系统有机地演变,规模和复杂性不断扩大,陷入僵局的风险增加,其影响可能更加严重:即使只是少数服务陷入僵局,许多其他服务在等待僵局反应时也可能受阻。结果,僵局的“核心”可能因其对系统其余部分的影响而被混淆,而诊断和解决问题可能具有挑战性。在这项工作中,我们提出在每种服务同时部署的分布式黑盒监测器,通过只观察进出的信息并与其他监测员交换探寻僵局,来应对挑战。我们提出了一个正式模型,捕捉到基于RPC的应用模式(例如Erlan/OTP的gen_服务器),以及一个分布式的黑盒监测算法,我们用不假阳性或假负性的方式找出了僵局。我们用DDDMF的检测工具来检测Erang/OB的检测结果,我们用这个模型来进行正式的检测。

---

## Article 5
### Title@2025-08-21 (4): Mini-Batch Robustness Verification of Deep Neural Networks

**Title**: Mini-Batch Robustness Verification of Deep Neural Networks | Mini-Batch Robustheitsprüfung von tiefen neuralen Netzwerken | 深神经网络微型批量强力核查 [2508.15454v1](http://arxiv.org/abs/2508.15454v1)

**Authors** (2): Saar Tzour-Shaday, Dana Drachsler Cohen

Neural network image classifiers are ubiquitous in many safety-critical applications. However, they are susceptible to adversarial attacks. To understand their robustness to attacks, many local robustness verifiers have been proposed to analyze $\epsilon$-balls of inputs. Yet, existing verifiers introduce a long analysis time or lose too much precision, making them less effective for a large set of inputs. In this work, we propose a new approach to local robustness: group local robustness verification. The key idea is to leverage the similarity of the network computations of certain $\epsilon$-balls to reduce the overall analysis time. We propose BaVerLy, a sound and complete verifier that boosts the local robustness verification of a set of $\epsilon$-balls by dynamically constructing and verifying mini-batches. BaVerLy adaptively identifies successful mini-batch sizes, accordingly constructs mini-batches of $\epsilon$-balls that have similar network computations, and verifies them jointly. If a mini-batch is verified, all $\epsilon$-balls are proven robust. Otherwise, one $\epsilon$-ball is suspected as not being robust, guiding the refinement. In the latter case, BaVerLy leverages the analysis results to expedite the analysis of that $\epsilon$-ball as well as the other $\epsilon$-balls in the batch. We evaluate BaVerLy on fully connected and convolutional networks for MNIST and CIFAR-10. Results show that BaVerLy scales the common one by one verification by 2.3x on average and up to 4.1x, in which case it reduces the total analysis time from 24 hours to 6 hours.

在许多安全关键应用程序中, 神经网络图像分类器是无处不在的。 但是, 它们很容易受到对抗性攻击。 为了了解它们对于攻击的稳健性, 许多本地强力核查器被提议分析 $\ epsilon$ balls 输入的球体。 然而, 现有的校验器引入了很长的分析时间, 或丢失了太多的精确度, 使得它们对大量输入器来说效果较差。 在这项工作中, 我们提出了一种新的地方稳健性方法: 集体稳健性核查。 关键的想法是利用网络计算的某些美元- balls 的类似性计算方法来减少总体分析时间。 我们提议BaVerLy, 一个可靠和完整的校准器, 通过动态的构建和校准, 美元- ballsalx 的校验结果, 通过常规的1美元- ballsalx 分析, 显示Uniceral- ballsalal- balls, 显示Uroughtal- sal- rubal- lax ex ex exal exal ex ex ex exal- lax ex ex ex ex ex ex lax. ex ex ex lax ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex exlevolut ex laut laut laut laut laut laut laut laut ex ex ex ex ex ex lax ex ex ex ex ex laut ex ex ex ex laut ex laut laut laut ex ex ex ex ex ex ex ex ex ex ex ex ex ex exal ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex ex

---

## Article 6
### Title@2025-08-21 (4): Fair Termination for Resource-Aware Active Objects

**Title**: Fair Termination for Resource-Aware Active Objects | Faire Beendigung für ressourcenschonende aktive Objekte | 资源软件活动对象的公平终止 [2508.15333v1](http://arxiv.org/abs/2508.15333v1)

**Authors** (4): Francesco Dagnino, Paola Giannini, Violet Ka I Pun, Ulises Torrella

Active object systems are a model of distributed computation that has been adopted for modelling distributed systems and business process workflows. This field of modelling is, in essence, concurrent and resource-aware, motivating the development of resource-aware formalisations on the active object model. The contributions of this work are the development of a core calculus for resource-aware active objects together with a type system ensuring that well-typed programs are fairly terminating, i.e., they can always eventually terminate. To achieve this, we combine techniques from graded semantics and type systems, which are quite well understood for sequential programs, with those for fair termination, which have been developed for synchronous~sessions.

活动对象系统是用于模拟分布式分布式系统和业务流程工作流程的分布式计算模型。这个模拟领域实质上是同时的和资源意识的,鼓励在活动对象模型上开发有资源意识的正规化。这项工作的贡献是为资源意识活跃对象开发一个核心计算法,同时开发一种类型系统,确保类型良好的程序能够公平终止,即它们能够最终终止。为了实现这一点,我们把分级的语义和类型系统的技术结合起来,这些技术在相继程序中得到相当理解,与为同步流取而开发的公平终止技术相结合。

---

## Article 7
### Title@2025-08-21 (4): Exploring the Theory and Practice of Concurrency in the   Entity-Component-System Pattern

**Title**: Exploring the Theory and Practice of Concurrency in the   Entity-Component-System Pattern | Erforschung der Theorie und Praxis der Übereinstimmung im Entity-Component-System-Muster | 探讨实体-构成部分-系统模式中的货币统一理论和实践 [2508.15264v1](http://arxiv.org/abs/2508.15264v1)

**Authors** (4): Patrick Redmond, Jonathan Castello, José Manuel Calderón Trilla, Lindsey Kuper

The Entity-Component-System (ECS) software design pattern, long used in game development, encourages a clean separation of identity (entities), data properties (components), and computational behaviors (systems). Programs written using the ECS pattern are naturally concurrent, and the pattern offers modularity, flexibility, and performance benefits that have led to a proliferation of ECS frameworks. Nevertheless, the ECS pattern is little-known and not well understood outside of a few domains. Existing explanations of the ECS pattern tend to be mired in the concrete details of particular ECS frameworks, or they explain the pattern in terms of imperfect metaphors or in terms of what it is not. We seek a rigorous understanding of the ECS pattern via the design of a formal model, Core ECS, that abstracts away the details of specific implementations to reveal the essence of software using the ECS pattern. We identify a class of Core ECS programs that behave deterministically regardless of scheduling, enabling use of the ECS pattern as a deterministic-by-construction concurrent programming model. With Core ECS as a point of comparison, we then survey several real-world ECS frameworks and find that they all leave opportunities for deterministic concurrency unexploited. Our findings point out a space for new ECS implementation techniques that better leverage such opportunities.

实体-功能-系统(ECS)软件设计模式长期用于游戏开发,鼓励将身份(实体)、数据属性(组成部分)和计算行为(系统)进行清洁分离。使用ECS模式编写的方案自然是并行的,该模式提供了模块性、灵活性和绩效效益,导致ECS框架的扩散。然而,ECS模式鲜为人知,在少数领域之外也没有很好理解。ECS模式的现有解释往往被某些ECS框架的具体细节所掩盖,或者它们以不完善的隐喻或不完善的隐喻(组成部分)和计算行为(系统)来解释其模式。我们寻求通过设计一种正式模式(核心ECS)来严格理解ECS模式的模式,该模式将具体实施的细节摘要用来揭示使用ECS框架软件的本质。我们确定了一种核心ECS方案类别,这种核心方案无论时间安排如何,都具有威慑性,能够将ECS模式用作一种确定性并同时构建的编程模型。通过核心ECS作为比较点,我们然后通过设计一些确定各种现实-机会,从而确定利用ECS的各种实际-机会。

---

## Article 8
### Title@2025-08-21 (4): Encode the $\forall\exists$ Relational Hoare Logic into Standard Hoare   Logic

**Title**: Encode the $\forall\exists$ Relational Hoare Logic into Standard Hoare   Logic | Kodieren Sie die $\forall\exists$ Relational Hoare Logic in Standard Hoare Logic | 将 $\ forall\ diverses $lational Hoare 逻辑编码为标准 Hoare 逻辑 [2504.17444v2](http://arxiv.org/abs/2504.17444v2)

**Authors** (3): Shushu Wu, Xiwei Wu, Qinxiang Cao

Verifying a real-world program's functional correctness can be decomposed into (1) a refinement proof showing that the program implements a more abstract high-level program and (2) an algorithm correctness proof at the high level. Relational Hoare logic serves as a powerful tool to establish refinement but often necessitates formalization beyond standard Hoare logic. Particularly in the nondeterministic setting, the $\forall\exists$ relational Hoare logic is required. Existing approaches encode this logic into a Hoare logic with ghost states and invariants, yet these extensions significantly increase formalization complexity and soundness proof overhead. This paper proposes a generic encoding theory that reduces the $\forall\exists$ relational Hoare logic to standard (unary) Hoare logic. Precisely, we propose to redefine the validity of relational Hoare triples while preserving the original proof rules and then encapsulate the $\forall\exists$ pattern within assertions. We have proved that the validity of encoded standard Hoare triples is equivalent to the validity of the desired relational Hoare triples. Moreover, the encoding theory demonstrates how common relational Hoare logic proof rules are indeed special cases of standard Hoare logic proof rules, and relational proof steps correspond to standard proof steps. Our theory enables standard Hoare logic to prove $\forall\exists$ relational properties by defining a predicate Exec, without requiring modifications to the logic framework or re-verification of soundness.

校验真实世界程序功能正确性的逻辑可以被分解为:(1) 一个精细的证明,表明该程序执行了一个更抽象的高层次程序,(2) 一个高层次的算法正确性证明。 通情 Hoare逻辑是建立完善的有力工具,但往往需要超越标准的Hoare逻辑, 特别是在非确定性背景中, 需要用美元来将这个逻辑与鬼魂国家和异族混杂的Hoare逻辑混为一流的现有方法, 但是这些扩展会大大增加正规化的复杂性和健全性证明管理。 本文提出了一个通用的编码理论, 将美元/ 美元/ 代谢性与Hoare逻辑逻辑化的逻辑化逻辑化降低到标准( unary) 标准逻辑化逻辑化逻辑化逻辑化逻辑化逻辑化。 我们建议重新定义Hoare三重定关系, 要求我们的标准逻辑化逻辑化规则的逻辑化与标准性框架的正确性框架的正确性。 我们证明Hoareal逻辑化逻辑化逻辑化逻辑性关系确实证明Hoal逻辑性关系。

---

## Article 9
### Title@2025-08-21 (4): Probabilistic Inference for Datalog with Correlated Inputs

**Title**: Probabilistic Inference for Datalog with Correlated Inputs | Probabilistische Schlussfolgerung für Datalog mit korrelierten Eingängen | 带有相关输入量的数据图表概率推断 [2508.15166v1](http://arxiv.org/abs/2508.15166v1)

**Authors** (5): Jingbo Wang, Shashin Halalingaiah, Weiyi Chen, Chao Wang, Isil Dillig

Probabilistic extensions of logic programming languages, such as ProbLog, integrate logical reasoning with probabilistic inference to evaluate probabilities of output relations; however, prior work does not account for potential statistical correlations among input facts. This paper introduces Praline, a new extension to Datalog designed for precise probabilistic inference in the presence of (partially known) input correlations. We formulate the inference task as a constrained optimization problem, where the solution yields sound and precise probability bounds for output facts. However, due to the complexity of the resulting optimization problem, this approach alone often does not scale to large programs. To address scalability, we propose a more efficient $\delta$-exact inference algorithm that leverages constraint solving, static analysis, and iterative refinement. Our empirical evaluation on challenging real-world benchmarks, including side-channel analysis, demonstrates that our method not only scales effectively but also delivers tight probability bounds.

ProbLog等逻辑编程语言的概率扩展,将逻辑推理与概率推论结合起来,以评价产出关系的概率;然而,先前的工作没有考虑到输入事实之间潜在的统计相关性。本文介绍了Praline,这是Datalog的一个新的扩展,目的是在(部分已知的)输入相关情况下精确概率推论。我们把推论任务作为一个有限的优化问题来设计,因为解决方案产生出合理和精确的输出事实的概率界限。然而,由于由此产生的优化问题的复杂性,仅此方法往往不向大型程序推广。为了解决可缩放性问题,我们建议一种更高效的 $\dela$-exact 推论算法,以利用制约解、静态分析和迭接式改进的制约。我们对挑战性现实世界基准的经验评估,包括侧道分析,表明我们的方法不仅有效,而且还提供了近乎概率的界限。

---

## Article 10
### Title@2025-08-21 (4): Big-Stop Semantics: A Simple Way to Get the Benefits of Small-Step   Semantics in a Big-Step Judgment

**Title**: Big-Stop Semantics: A Simple Way to Get the Benefits of Small-Step   Semantics in a Big-Step Judgment | Big-Stop Semantics: Ein einfacher Weg, um die Vorteile von Small-Step Semantics in einem Big-Step Urteil zu bekommen | 大站式语义学:在大站式判决中 获得小型语义学好处的简单方法 [2508.15157v1](http://arxiv.org/abs/2508.15157v1)

**Authors** (3): David M Kahn, Jan Hoffmann, Runming Li

As evident in the programming language literature, many practitioners favor specifying dynamic program behavior using big-step over small-step semantics. Unlike small-step semantics, which must dwell on every intermediate program state, big-step semantics conveniently jump directly to the ever-important result of the computation. Big-step semantics also typically involve fewer inference rules than their small-step counterparts. However, in exchange for ergonomics, big-step semantics give up power: Small-step semantics describes program behaviors that are outside the grasp of big-step semantics, notably divergence. This work presents a little-known extension of big-step semantics with inductive definitions that captures diverging computations without introducing error states. This big-stop semantics is illustrated for typed, untyped, and effectful variants of PCF, as well as a while-loop-based imperative language. Big-stop semantics extends the standard big-step inference rules with a few additional rules to define an evaluation judgment that is equivalent to the reflexive-transitive closure of small-step transitions. This simple extension contrasts with other solutions in the literature which sacrifice ergonomics by introducing many additional inference rules, global state, and/or less-commonly-understood reasoning principles like coinduction.

正如编程语言文献所显示的,许多实践者都倾向于使用跨步小步语义来指定动态程序行为。 与小步语义(必须停留在每个中间程序状态)不同的是,大步语义(必须停留在每一个中间程序状态 ) 与小步语义(必须停留在每一个中间程序状态 ) 不同, 大步语义( 粗步语义) 直接跳跃到计算中的重要结果。 大步语义( 粗步语义) 通常也涉及比小步语义( 小步语义) 更少的推导规则。 但是, 大步语义( 大步语义) 的推论( 大步语义) , 描述在大步语义( 大步语义, 特别是差异 ) 之外的程序行为( ) 。 这项工作是一个鲜为人知的大步语义语义( ) 语义( 大步语义) , 其含义( ) 和 隐含性( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) ( ) (

---

## Article 11
### Title@2025-08-21 (4): Software Model Checking via Summary-Guided Search (Extended Version)

**Title**: Software Model Checking via Summary-Guided Search (Extended Version) | Software-Modellprüfung über Zusammenfassungs-geführte Suche (erweiterte Version) | 通过简易辅助搜索(扩展版本)核对软件模型 [2508.15137v1](http://arxiv.org/abs/2508.15137v1)

**Authors** (3): Ruijie Fang, Zachary Kincaid, Thomas Reps

In this work, we describe a new software model-checking algorithm called GPS. GPS treats the task of model checking a program as a directed search of the program states, guided by a compositional, summary-based static analysis. The summaries produced by static analysis are used both to prune away infeasible paths and to drive test generation to reach new, unexplored program states. GPS can find both proofs of safety and counter-examples to safety (i.e., inputs that trigger bugs), and features a novel two-layered search strategy that renders it particularly efficient at finding bugs in programs featuring long, input-dependent error paths. To make GPS refutationally complete (in the sense that it will find an error if one exists, if it is allotted enough time), we introduce an instrumentation technique and show that it helps GPS achieve refutation-completeness without sacrificing overall performance. We benchmarked GPS on a suite of benchmarks including both programs from the Software Verification Competition (SV-COMP) and from prior literature, and found that our implementation of GPS outperforms state-of-the-art software model checkers (including the top performers in SV-COMP ReachSafety-Loops category), both in terms of the number of benchmarks solved and in terms of running time.

在这项工作中,我们描述了一个新的软件模型检查算法,称为GPS。GPS将模型检查程序的任务视为对程序状态的定向搜索,以基于摘要的合成静态分析为指导。静态分析产生的摘要被用于缩小不可行的路径,并驱动测试生成达到新的、未探索的程序状态。GPS可以找到安全的证据和安全(即触发错误的投入)的反抽样(即触发错误的投入),并具有一种新的双层搜索战略,使得它能够特别高效地发现存在长、基于输入的错误路径的程序中的错误。要让GPS倒置完整(也就是说,如果存在错误,就会发现错误,如果分配的时间够长,就会发现错误),我们引入了一种仪表技术,并表明它有助于GPS在不牺牲总体性能的情况下实现反驳的完整。我们根据一套基准,包括软件核查竞赛(SV-COMP)和先前文献中的程序,发现我们执行的GPSBS-Mo-AF软件的州级标准,包括SRAFS-SL标准中执行的顶级标准。

---

## Article 12
### Title@2025-08-20 (3): Homomorphism Calculus for User-Defined Aggregations

**Title**: Homomorphism Calculus for User-Defined Aggregations | Homomorphismus Calculus für benutzerdefinierte Aggregationen | 用户定义聚合的自定义聚合计算 [2508.15109v1](http://arxiv.org/abs/2508.15109v1)

**Authors** (5): Ziteng Wang, Ruijie Fang, Linus Zheng, Dixin Tang, Isil Dillig

Data processing frameworks like Apache Spark and Flink provide built-in support for user-defined aggregation functions (UDAFs), enabling the integration of domain-specific logic. However, for these frameworks to support \emph{efficient} UDAF execution, the function needs to satisfy a \emph{homomorphism property}, which ensures that partial results from independent computations can be merged correctly. Motivated by this problem, this paper introduces a novel \emph{homomorphism calculus} that can both verify and refute whether a UDAF is a dataframe homomorphism. If so, our calculus also enables the construction of a corresponding merge operator which can be used for incremental computation and parallel execution. We have implemented an algorithm based on our proposed calculus and evaluate it on real-world UDAFs, demonstrating that our approach significantly outperforms two leading synthesizers.

Apache Spark 和 Flink 等数据处理框架为用户定义的聚合功能提供了内在支持, 从而能够整合特定域的逻辑。 但是, 这些框架要支持 emph{ valid} UDAF 执行, 功能需要满足 emph{ homorphismis 属性 , 以确保独立计算得出的部分结果能够正确合并 。 受这一问题的驱动, 本文引入了一个新的 \ emph{ homommory callulus} , 它既可以验证, 也可以反驳 UDAF 是否是一个数据框架的共性。 如果是这样, 我们的微积分也能够构建一个相应的合并操作器, 用于递增计算和平行执行 。 我们已经根据我们提议的微积分进行了算, 并用真实世界的UDAF 进行了评估。 这表明我们的方法大大优于两个主要的合成器 。

---

## Article 13
### Title@2025-08-20 (3): Choreographic Quick Changes: First-Class Location (Set) Polymorphism

**Title**: Choreographic Quick Changes: First-Class Location (Set) Polymorphism | Choreographische schnelle Änderungen: Standort der ersten Klasse (Set) Polymorphismus | 舞蹈快速变化:第一类位置(Set)多形态 [2506.10913v2](http://arxiv.org/abs/2506.10913v2)

**Authors** (3): Ashley Samuelson, Andrew K. Hirsch, Ethan Cecchetti

Choreographic programming is a promising new paradigm for programming concurrent systems where a developer writes a single centralized program that compiles to individual programs for each node. Existing choreographic languages, however, lack critical features integral to modern systems, like the ability of one node to dynamically compute who should perform a computation and send that decision to others. This work addresses this gap with $\lambda_{QC}$, the first typed choreographic language with \emph{first class process names} and polymorphism over both types and (sets of) locations. $\lambda_{QC}$ also improves expressive power over previous work by supporting algebraic and recursive data types as well as multiply-located values. We formalize and mechanically verify our results in Rocq, including the standard choreographic guarantee of deadlock freedom.

舞蹈编程是编程并行系统的一个有希望的新范例,在编程中,一位开发者为每个节点的单个程序编译一个单一的集中程序。然而,现有的舞蹈语言缺乏现代系统所不可或缺的关键特征,例如一个节点能够动态计算谁应进行计算并将决定发送给其他人。这项工作用美元($)解决了这一差距,这是首种带有 emph{ 一级进程名称的舞蹈语言,以及两种类型和(类)地点的多元形态主义。$(lambda}C})也通过支持代数和循环数据类型以及倍数定位值,提高了对以往工作的表达力。我们正式和机械地核查了我们在罗克(Rocq)中的结果,包括僵局自由的标准舞蹈保障。

---

## Article 14
### Title@2025-08-20 (3): Reasoning about Weak Isolation Levels in Separation Logic

**Title**: Reasoning about Weak Isolation Levels in Separation Logic | Begründung schwacher Isolationsebenen in der Trennungslogik | 分离逻辑中薄弱隔离水平的原因 [2501.14421v2](http://arxiv.org/abs/2501.14421v2)

**Authors** (5): Anders Alnor Mathiasen, Léon Gondelman, Léon Ducruet, Amin Timany, Lars Birkedal

Consistency guarantees among concurrently executing transactions in local- and distributed systems, commonly referred to as isolation levels, have been formalized in a number of models. Thus far, no model can reason about executable implementations of databases or local transaction libraries providing weak isolation levels. Weak isolation levels are characterized by being highly concurrent and, unlike their stronger counterpart serializability, they are not equivalent to the consistency guarantees provided by a transaction library implemented using a global lock. Industrial-strength databases almost exclusively implement weak isolation levels as their default level. This calls for formalism as numerous bugs violating isolation have been detected in these databases. In this paper, we formalize three weak isolation levels in separation logic, namely read uncommitted, read committed, and snapshot isolation. We define modular separation logic specifications that are independent of the underlying transaction library implementation. Historically, isolation levels have been specified using examples of executions between concurrent transactions that are not allowed to occur, and we demonstrate that our specifications correctly prohibit such examples. To show that our specifications are realizable, we formally verify that an executable implementation of a key-value database running the multi-version concurrency control algorithm from the original snapshot isolation paper satisfies our specification of snapshot isolation. Moreover, we prove implications between the specifications -- snapshot isolation implies read committed and read committed implies read uncommitted -- and thus the verification effort of the database serves as proof that all of our specifications are realizable. All results are mechanized in the Rocq proof assistant on top of the Iris separation logic framework.

在本地和分布式系统中同时执行交易(通常称为隔离水平)之间的一致性保障,在一些模式中已经正式确定,迄今为止,没有任何模式可以说明如何以可执行的方式执行数据库或地方交易图书馆,而隔离程度较低,其特点是高度同时,与其更强的对应序列性不同,它们不等同于使用全球锁执行的交易图书馆所提供的一致性保障。工业强度数据库几乎完全将薄弱的隔离水平作为其默认水平。这要求正式主义,因为在这些数据库中发现了许多违反隔离的错误。在本文件中,我们正式确定了分离逻辑中三种薄弱的隔离程度,即阅读不承诺、阅读和快照隔离。我们定义模块隔离逻辑的特点是高度同时执行,其特征与与之相对应的序列性不同,其特征与使用的全球锁执行交易图书馆所提供的一致性保障不同,它们不等同于使用同步交易图书馆所提供的一致性保障。我们的技术规格正确地禁止了此类例子。为了表明我们的规格是真实的,我们正式核实了如何执行一个关键价值数据库,运行多版本的隔离性货币控制标准,即阅读了未承诺的孤立性记录,从而证明了我们最初的孤立性文件的准确性标准。

---

## Article 15
### Title@2025-08-20 (3): Gradual Metaprogramming

**Title**: Gradual Metaprogramming | Stufenweise Metaprogrammierung | 渐进元元方案 [2506.09043v3](http://arxiv.org/abs/2506.09043v3)

**Authors** (7): Tianyu Chen, Darshal Shetty, Jeremy G. Siek, Chao-Hong Chen, Weixi Ma, Arnaud Venet, Rocky Liu

Data engineers increasingly use domain-specific languages (DSLs) to generate the code for data pipelines. Such DSLs are often embedded in Python. Unfortunately, there are challenges in debugging the generation of data pipelines: an error in a Python DSL script is often detected too late, after the execution of the script, and the source code location that triggers the error is hard to pinpoint.   In this paper, we focus on the scenario where a DSL embedded in Python (so it is dynamically-typed) generates data pipeline description code that is statically-typed. We propose gradual metaprogramming to (1) provide a migration path toward statically typed DSLs, (2) immediately provide earlier detection of code generation type errors, and (3) report the source code location responsible for the type error. Gradual metaprogramming accomplishes this by type checking code fragments and incrementally performing runtime checks as they are spliced together. We define MetaGTLC, a metaprogramming calculus in which a gradually-typed metalanguage manipulates a statically-typed object language, and give semantics to it by translation to the cast calculus MetaCC. We prove that successful metaevaluation always generates a well-typed object program and mechanize the proof in Agda.

数据工程师越来越多地使用特定域语言( DSLs ) 生成数据管道代码。 这些 DSL 通常嵌入 Python 。 不幸的是, 在调试数据管道生成中存在挑战: 执行脚本后, Python DSL 脚本中的错误往往被检测得太晚, 触发错误的源代码位置很难定位 。 在本文中, 我们侧重于在 Python 中嵌入 DSL( 因而是动态型式的) 生成静态型数据管道描述代码的情景。 我们建议渐进式元程序化:(1) 提供一条通往静态键入 DSL 的迁移路径, (2) 立即提供早期代码生成类型错误的检测, (3) 报告类型错误的源代码位置。 渐进式元程序通过类型检查代码片段和在同步时逐步进行运行检查来完成这一点 。 我们定义了MetGLC, 元程序化的计算公式化计算方法, 在其中, 将目标逐渐类型化的元化的元化地对静态类型对象进行翻译, 将最终的模模版版版式翻译。

---

## Article 16
### Title@2025-08-20 (3): Proto-Quipper with Reversing and Control

**Title**: Proto-Quipper with Reversing and Control | Proto-Quipper mit Reversing und Kontrolle | 具有校正和控制功能的预置置置信器 [2410.22261v2](http://arxiv.org/abs/2410.22261v2)

**Authors** (4): Peng Fu, Kohei Kishida, Neil J. Ross, Peter Selinger

The quantum programming language Quipper supports circuit operations such as reversing and controlling certain quantum circuits. Additionally, Quipper provides a function called with-computed, which can be used to program circuits of the form g; f; g-dagger. The latter is a common pattern in quantum circuit design. One benefit of using with-computed, as opposed to constructing the circuit g ; f; g-dagger directly from g, f, and g-dagger, is that it facilitates an important optimization. Namely, if the resulting circuit is later controlled, only f needs to be controlled; the circuits g and g-dagger need not even be controllable.   In this paper, we formalize a semantics for reversible and controllable circuits, using a dagger symmetric monoidal category R to interpret reversible circuits, and a new notion we call a controllable category N, which encompasses the control and with-computed operations in Quipper. We extend the language Proto-Quipper with reversing, control and the with-computed operation. Since not all circuits are reversible and/or controllable, we use a type system with modalities to track reversibility and controllability. This generalizes the modality of Fu-Kishida-Ross-Selinger 2023. We give an abstract categorical semantics, and show that the type system and operational semantics are sound with respect to this semantics.

量子编程语言 Quipper 支持电路操作, 如反转和控制某些量子电路。 此外, Quipper 提供了一种功能, 它被调用为可调用, 可用于编程表g; f; g- 制动器。 后者是量子电路设计中常见的模式。 使用量子程序语言的一个好处, 而不是构建电路g; f; g- 和 g- 制动器, 就是它能够促进重要的优化。 也就是说, 如果由此产生的电路后来被控制, 只需要控制 f; 电路 g 和 g- 制动器甚至不需要控制。 在本文件中, 我们将可变换和可控的电路和可控电路的语系正式化, 使用直流模式来解释可变电路路路; 新的概念我们称之为可控的N类, 它包含控和在Quipper 中被调用的操作。 我们将语言 Proto- Quipper 与可逆、 控和可变码操作的可变码- 。 由于所有电路路模式, 和可变式控制, 这个系统是可变式的可变式系统可变和可变式 。

---

## Article 17
### Title@2025-08-20 (3): Reachability is Decidable for ATM-Typable Finitary PCF with Effect   Handlers

**Title**: Reachability is Decidable for ATM-Typable Finitary PCF with Effect   Handlers | Erreichbarkeit ist für ATM-Typable Finitary PCF mit Effekt-Handlern entscheidend | 对于带有效果操作器的自动取款机易失能性低调可打字软性PCF [2508.12572v2](http://arxiv.org/abs/2508.12572v2)

**Authors** (2): Ryunosuke Endo, Tachio Terauchi

It is well known that the reachability problem for simply-typed lambda calculus with recursive definitions and finite base-type values (finitary PCF) is decidable. A recent paper by Dal Lago and Ghyselen has shown that the same problem becomes undecidable when the language is extended with algebraic effect and handlers (effect handlers). We show that, perhaps surprisingly, the problem becomes decidable even with effect handlers when the type system is extended with answer type modification (ATM). A natural intuition may find the result contradictory, because one would expect allowing ATM makes more programs typable. Indeed, this intuition is correct in that there are programs that are typable with ATM but not without it, as we shall show in the paper. However, a corollary of our decidability result is that the converse is true as well: there are programs that are typable without ATM but becomes untypable with ATM, and we will show concrete examples of such programs in the paper. Our decidability result is proven by a novel continuation passing style (CPS) transformation that transforms an ATM-typable finitary PCF program with effect handlers to a finitary PCF program without effect handlers. Additionally, as another application of our CPS transformation, we show that every recursive-function-free ATM-typable finitary PCF program with effect handlers terminates, while there are (necessarily ATM-untypable) recursive-function-free finitary PCF programs with effect handlers that may diverge. Finally, we disprove a claim made in a recent work that proved a similar but strictly weaker decidability result. We foresee our decidability result to lay a foundation for developing verification methods for programs with effect handlers, just as the decidability result for reachability of finitary PCF has done such for programs without effect handlers.

众所周知, 简单类型 lambda 计算器( filitary PCF) 的可达性问题可以判断。 Dal Lago 和 Ghyselen 最近的论文显示, 当语言使用代数效果和处理器( 效果处理器) 扩展语言时, 同样的问题将无法确定。 我们显示, 可能令人惊讶的是, 当类型系统通过答题类型修改( ATM ) 扩展时, 问题甚至有效处理器也会发生衰变。 自然直觉可能会发现结果自相矛盾, 因为人们预期允许 ATM 会让更多的程序可复制。 事实上, 这种直觉是正确的, 因为有些程序可以用 ATM 和 Ghyselsen 来复制, 但是我们会在纸质变效果中显示, 我们的变变现程序会变变变变变。 顺序程序的必然结果是: 没有 ATM 的可调缩写功能变更精确性程序, 我们的功能变变异性程序会变变变变变。

---

## Article 18
### Title@2025-08-20 (3): Close is Good Enough: Component-Based Synthesis Modulo Logical   Similarity

**Title**: Close is Good Enough: Component-Based Synthesis Modulo Logical   Similarity | Schließen ist gut genug: Komponentenbasierte Synthese Modulo Logische Ähnlichkeit | 关闭为良好 足够 : 基于组件的合成模型逻辑相似性 [2508.14614v1](http://arxiv.org/abs/2508.14614v1)

**Authors** (2): Ashish Mishra, Suresh Jagannathan

Component-based synthesis (CBS) aims to generate loop-free programs from a set of libraries whose methods are annotated with specifications and whose output must satisfy a set of logical constraints, expressed as a query. The effectiveness of a CBS algorithm critically depends on the severity of the constraints imposed by the query. The more exact these constraints are, the sparser the space of feasible solutions. This maxim also applies when we enrich the expressiveness of the specifications affixed to library methods. In both cases, the search must now contend with constraints that may only hold over a small number of the possible execution paths that can be enumerated by a CBS procedure.   In this paper, we address this challenge by equipping CBS search with the ability to reason about logical similarities among the paths it explores. Our setting considers library methods equipped with refinement-type specifications that enrich ordinary base types with a set of rich logical qualifiers to constrain the set of values accepted by that type. We perform a search over a tree automata variant called Qualified Tree Automata that intelligently records information about enumerated terms, leveraging subtyping constraints over the refinement types associated with these terms to enable reasoning about similarity among candidate solutions as search proceeds, thereby avoiding exploration of semantically similar paths.   We present an implementation of this idea in a tool called \name, and provide a comprehensive evaluation that demonstrates \name's ability to synthesize solutions to complex CBS queries that go well-beyond the capabilities of the existing state-of-the-art.

基于组件的合成( CBS) 旨在从一组图书馆生成无环程序,这些图书馆的方法配有规格说明,其产出必须满足一系列逻辑限制,以查询方式表达。 CBS 算法的有效性关键取决于查询所施加的限制的严厉程度。 这些限制越精确,就越少可行解决办法的空间。 当我们丰富图书馆方法所附带的规格的清晰度时, 也适用这个符号。 在这两种情况下, 搜索现在必须遇到一些限制, 这些限制只能维持在CBS 程序所列举的少量可能的执行路径上。 在本文中, 我们应对这一挑战, 使 CBS 搜索具备解释其所探索路径的逻辑相似性的能力。 我们的设置考虑到具有精细型规格的图书馆方法, 使普通的基础类型更加丰富, 并配有一套丰富的逻辑修饰符, 以限制该类型所接受的一套价值。 我们对树自动变体进行搜索, 称为“ 质化树自动变体” , 以智能的方式记录所罗列的术语, 利用与精细的精细的解决方案类型相关的亚限制。 我们的CBS- 将这种精细的精细的精细的精细的精细的精选能力定位作为推。

---

## Article 19
### Title@2025-08-20 (3): Tuning Random Generators: Property-Based Testing as Probabilistic   Programming

**Title**: Tuning Random Generators: Property-Based Testing as Probabilistic   Programming | Tuning Random Generators: Eigenschaftsbasierte Prüfung als probabilistische Programmierung | 测算随机发电机:以财产为基础的测试作为概率规划 [2508.14394v1](http://arxiv.org/abs/2508.14394v1)

**Authors** (6): Ryan Tjoa, Poorva Garg, Harrison Goldstein, Todd Millstein, Benjamin Pierce, Guy Van den Broeck

Property-based testing validates software against an executable specification by evaluating it on randomly generated inputs. The standard way that PBT users generate test inputs is via generators that describe how to sample test inputs through random choices. To achieve a good distribution over test inputs, users must tune their generators, i.e., decide on the weights of these individual random choices. Unfortunately, it is very difficult to understand how to choose individual generator weights in order to achieve a desired distribution, so today this process is tedious and limits the distributions that can be practically achieved.   In this paper, we develop techniques for the automatic and offline tuning of generators. Given a generator with undetermined symbolic weights and an objective function, our approach automatically learns values for these weights that optimize for the objective. We describe useful objective functions that allow users to (1) target desired distributions and (2) improve the diversity and validity of their test cases. We have implemented our approach in a novel discrete probabilistic programming system, Loaded Dice, that supports differentiation and parameter learning, and use it as a language for generators. We empirically demonstrate that our approach is effective at optimizing generator distributions according to the specified objective functions. We also perform a thorough evaluation on PBT benchmarks, demonstrating that, when automatically tuned for diversity and validity, the generators exhibit a 3.1-7.4x speedup in bug finding.

以财产为基础的测试通过对随机生成的投入进行评估,对照可执行的规格来验证软件。 PBT 用户生成测试投入的标准方法是通过描述如何通过随机选择抽样测试投入的发电机。为了在测试投入中实现良好的分配,用户必须调整其发电机的重量,即决定这些个别随机选择的重量。不幸的是,很难理解如何选择单个发电机的重量,以实现理想的分布,因此,今天这个过程是乏味的,限制了可以实际实现的分布。在本文中,我们开发了发电机自动和离线调整技术。鉴于发电机有不确定的符号重量和客观功能,我们的方法自动学习这些重量的价值,以达到最佳目标。我们描述有用的客观功能,使用户(1) 瞄准理想的分布,(2) 提高测试案例的多样性和有效性。我们在一个新颖的离散的概率性编程系统(loaddd Dice)中实施了我们的方法,支持差异化和参数化学习,并将它用作发电机的语言。我们实验性地表明,我们的方法在优化发电机的分布速度时,我们用一个目标性的标准是,我们用在最精确地显示,我们的方法在最精确地展示发动机分布的进度上。

---

## Article 20
### Title@2025-08-19 (2): Prompt Orchestration Markup Language

**Title**: Prompt Orchestration Markup Language | Prompt Orchestrierung Markup Sprache | 提示管管标记语言 [2508.13948v1](http://arxiv.org/abs/2508.13948v1)

**Authors** (4): Yuge Zhang, Nan Chen, Jiahang Xu, Yuqing Yang

Large Language Models (LLMs) require sophisticated prompting, yet current practices face challenges in structure, data integration, format sensitivity, and tooling. Existing methods lack comprehensive solutions for organizing complex prompts involving diverse data types (documents, tables, images) or managing presentation variations systematically. To address these gaps, we introduce POML (Prompt Orchestration Markup Language). POML employs component-based markup for logical structure (roles, tasks, examples), specialized tags for seamless data integration, and a CSS-like styling system to decouple content from presentation, reducing formatting sensitivity. It includes templating for dynamic prompts and a comprehensive developer toolkit (IDE support, SDKs) to improve version control and collaboration. We validate POML through two case studies demonstrating its impact on complex application integration (PomLink) and accuracy performance (TableQA), as well as a user study assessing its effectiveness in real-world development scenarios.

大语言模型(LLMS)需要精密的催化,但目前的做法在结构、数据整合、格式敏感性和工具方面面临着挑战。现有方法缺乏组织涉及不同类型数据(文件、表格、图像)或系统管理演示式变异的复杂提示的全面解决办法。为弥补这些差距,我们引入POML(快速交响标记语言)。POML为逻辑结构(作用、任务、实例)、无缝数据整合专用标记和CSS相似的搭配系统,以将内容与演示文稿脱钩,降低格式敏感性。它包括动态提示和综合开发工具工具包(IDE支持,SDKs),以改进版本控制和协作。我们通过两个案例研究来验证POMLM(PomLink)及其对复杂应用整合(PomLink)和准确性性能(TableQA)的影响,以及用户评估其在现实世界发展情景中的有效性的研究。

---

## Article 21
### Title@2025-08-19 (2): Bisimilarity and Simulatability of Processes Parameterized by Join   Interactions

**Title**: Bisimilarity and Simulatability of Processes Parameterized by Join   Interactions | Bisimilarität und Simulatierbarkeit von Prozessen, die durch Join Interactions parametriert werden | 联合互动所显示的进程参数的双相性和可模拟性 [2508.13611v1](http://arxiv.org/abs/2508.13611v1)

**Authors** (2): Clemens Grabmayer, Maurizio Murgia

Departing from Larsen's concept of parameterized bisimilarity of processes with respect to interaction with environments, we start an exploration of its natural weakening: bisimilarity of unrestricted join interactions with environments. Parameterized bisimilarity relates processes p and q with respect to an environment e if p and q behave bi-similarly while joining -- respectively the same -- transitions from e. The weakened variant relates processes p and q with respect to environment e if the join-interaction processes p & e and q & e of p and q with e are bisimilar. (Hereby join interactions r & f facilitate a step with label a to r' & f' if and only if r and f permit a-steps to r' and f' , respectively.) Join-interaction parameterized (ji-parameterized) bisimilarity coincides with parameterized bisimilarity for deterministic environments, but that it is a coarser equivalence in general. We explain how Larsen's concept can be recovered from ji-parameterized bisimilarity by 'determinizing' interactions. We show that by adaptation to simulatability (simulation preorder) the same concept arises: parameterized simulatability coincides with ji-parameterized simulatability. For the discrimination preorder of (ji-)parameterized simulatability on environments we obtain the same result as Larsen did for parameterized bisimilarity. Also, we give a modal-logic characterization of (ji-)parameterized simulatability. Finally we gather open problems, and provide an outlook on our current related work.

脱离Larsen关于进程在与环境互动方面的参数性两异的概念,我们开始探索其自然变弱:不受限制地与环境互动的两异性。参数性双异性在环境方面与进程 p 和 q 相关,如果 p 和 q 行为是相似的,同时加入 -- 从 e 的过渡。 被削弱的变量在环境方面与进程 p 和 q 相关,如果联合互动进程 p & e 和 q e 与 e 相关是两异的,则我们开始探索其自然变弱性。 (通过结合互动 r & f 来推动与 r' f 相关标签的一步,如果并且只有在 r 和 f 分别允许在 r' 和 f 之间出现一个步骤时。) 合并性参数化(ji- 参数化) 与确定性环境之前的参数性相匹配,但在一般情况下,Larsen' 概念如何从二分解的参数性中恢复 。 我们的精确性- 确定性 的精确性 的精确性, 我们的精确性 也通过精确性 定义性 的精确性 和精确性 来进行调整性 。

---

## Article 22
### Title@2025-08-19 (2): Reactive Semantics for User Interface Description Languages

**Title**: Reactive Semantics for User Interface Description Languages | Reaktive Semantik für Benutzeroberfläche Beschreibung Sprachen | 用户界面描述语言的自反应语义 [2508.13610v1](http://arxiv.org/abs/2508.13610v1)

**Authors** (3): Basile Pesin, Celia Picard, Cyril Allignol

User Interface Description Languages (UIDLs) are high-level languages that facilitate the development of Human-Machine Interfaces, such as Graphical User Interface (GUI) applications. They usually provide first-class primitives to specify how the program reacts to an external event (user input, network message), and how data flows through the program. Although these domain-specific languages are now widely used to implement safety-critical GUIs, little work has been invested in their formalization and verification.   In this paper, we propose a denotational semantic model for a core reactive UIDL, Smalite, which we argue is expressive enough to encode constructs from more realistic languages. This preliminary work may be used as a stepping stone to produce a formally verified compiler for UIDLs.

用户界面描述语言(UIDLs)是有助于开发人类-海洋界面的高级语言,例如图形用户界面(GUI)应用程序。它们通常提供一流原始语言,以具体说明程序如何对外部事件(用户输入、网络信息)作出反应,以及如何通过程序进行数据流。虽然这些特定域名语言现在被广泛用于实施安全关键界面,但在它们的正规化和验证方面几乎没有投入任何工作。在本文中,我们提议了一个用于核心反应性UIDL(Smarite)的注释性语义模型,我们认为该模型的表达性足以用更现实的语言进行编码构建。这一初步工作可用作制作经正式核实的 UIDLs 编集的跳板。

---

## Article 23
### Title@2025-08-18 (1): Strengthening Programming Comprehension in Large Language Models through   Code Generation

**Title**: Strengthening Programming Comprehension in Large Language Models through   Code Generation | Stärkung des Programmierverständnisses in großen Sprachmodellen durch Codegenerierung | 通过代码生成加强大语言模式的方案编制理解 [2508.12620v1](http://arxiv.org/abs/2508.12620v1)

**Authors** (7): Xiaoning Ren, Qiang Hu, Wei Ma, Yan Li, Yao Zhang, Lingxiao Jiang, Yinxing Xue

Large language models (LLMs) have recently shown impressive results on diverse code-related tasks, benefiting from large-scale training and instruction tuning. However, studies reveal that their grasp of fundamental programming concepts, such as data flow and control flow, remains shallow, leading to fragile performance when code requires deeper reasoning. This limitation restricts the practical adoption of LLMs in real-world software development. To address this issue, this work introduces a counterfactual code augmentation framework combined with concept-aware tuning, designed to guide LLMs toward stronger conceptual understanding. Comprehensive evaluation across multiple models and benchmarks demonstrates the effectiveness of the proposed approach.

大型语言模型(LLMs)最近从大规模培训和教学调整中受益,在与代码有关的任务中显示出令人印象深刻的成果,但是,研究表明,它们掌握的数据流动和控制流动等基本编程概念仍然很浅,导致在代码需要更深入推理时业绩脆弱。这一限制限制了实际采用LLMs开发现实世界软件。为解决这一问题,这项工作引入了一个反事实代码增强框架,并结合概念认知调整,旨在引导LLMs在概念上获得更强有力的理解。对多种模型和基准的全面评价显示了拟议方法的有效性。

---

## Article 24
### Title@2025-08-17 (7): Type-Driven Prompt Programming: From Typed Interfaces to a Calculus of   Constraints

**Title**: Type-Driven Prompt Programming: From Typed Interfaces to a Calculus of   Constraints | Typgetriebene Prompt-Programmierung: Von eingegebenen Schnittstellen zu einem Calculus of Constraints | 类型驱动快速编程: 从键入接口到制约的计算 [2508.12475v1](http://arxiv.org/abs/2508.12475v1)

**Authors** (1): Abhijit Paul

Prompt programming treats large language model prompts as software components with typed interfaces. Based on a literature survey of 15 recent works from 2023 to 2025, we observe a consistent trend: type systems are central to emerging prompt programming frameworks. However, there are gaps in constraint expressiveness and in supporting algorithms. To address these issues, we introduce the notion of Lambda Prompt, a dependently typed calculus with probabilistic refinements for syntactic and semantic constraints. While this is not yet a full calculus, the formulation motivates a type-theoretic foundation for prompt programming. Our catalog of 13 constraints highlights underexplored areas in constraint expressiveness (constraints 9 through 13). To address the algorithmic gap, we propose a constraint-preserving optimization rule. Finally, we outline research directions on developing a compiler for prompt programs.

根据对2023年至2025年15项近期工程的文献调查,我们观察到一个一致的趋势:类型系统是新出现的快速编程框架的核心。然而,在限制表达性和支持算法方面存在着差距。为了解决这些问题,我们提出了兰巴达快(Lambda Jear)概念,这是一个单打式的微积分,具有综合和语义限制的概率改进功能。虽然还不是完整的微积分,但这种配方为迅速编程提供了一种类型理论基础。我们共有13项限制因素的编目显示受限制的显性未得到充分探讨的领域(约束9至13)。为了解决算法差距,我们提出了限制保留优化规则。最后,我们概述了为快速编程开发一个编译器的研究方向。

---

## Article 25
### Title@2025-08-17 (7): Controlling Copatterns: There and Back Again (Extended Version)

**Title**: Controlling Copatterns: There and Back Again (Extended Version) | Kopatterns steuern: Es und wieder zurück (Erweiterte Version) | 控制集散器: 重置和倒置( 扩展版本) [2508.12427v1](http://arxiv.org/abs/2508.12427v1)

**Authors** (1): Paul Downen

Copatterns give functional programs a flexible mechanism for responding to their context, and composition can greatly enhance their expressiveness. However, that same expressive power makes it harder to precisely specify the behavior of programs. Using Danvy's functional and syntactic correspondence between different semantic artifacts, we derive a full suite of semantics for copatterns, twice. First, a calculus of monolithic copatterns is taken on a journey from small-step operational semantics to abstract machine to continuation-passing style. Then within continuation-passing style, we refactor the semantics to derive a more general calculus of compositional copatterns, and take the return journey back to derive the other semantic artifacts in reverse order.

Coptatters 给功能性程序提供了一个灵活的机制来应对其上下文, 组成可以大大增强它们的表达性。 但是, 同样的表达力使得更难精确地指定程序的行为。 我们利用 Danvy 的功能性和合成性对不同语义文物之间的对应, 我们得出一套完整的拼写词组, 两次。 首先, 独立型编织物的微积分是在从小步操作的语义到抽象机器到继续传承风格的旅程中进行的。 然后在继续的通俗风格中, 我们重新定义语义学, 以产生更普遍的拼写拼写词组的缩略图, 并选择返回的行程, 以逆序生成其他语义文物 。

---

## Article 26
### Title@2025-08-16 (6): Certified Compilation based on Gödel Numbers

**Title**: Certified Compilation based on Gödel Numbers | Zertifizierte Zusammenstellung basierend auf Gödel-Nummern | 基于哥德尔号的核证汇编 [2508.12054v1](http://arxiv.org/abs/2508.12054v1)

**Authors** (2): Guilherme de Oliveira Silva, Fernando Magno Quintão Pereira

In his 1984 Turing Award lecture, Ken Thompson showed that a compiler could be maliciously altered to insert backdoors into programs it compiles and perpetuate this behavior by modifying any compiler it subsequently builds. Thompson's hack has been reproduced in real-world systems for demonstration purposes. Several countermeasures have been proposed to defend against Thompson-style backdoors, including the well-known {\it Diverse Double-Compiling} (DDC) technique, as well as methods like translation validation and CompCert-style compilation. However, these approaches ultimately circle back to the fundamental question: "How can we trust the compiler used to compile the tools we rely on?" In this paper, we introduce a novel approach to generating certificates to guarantee that a binary image faithfully represents the source code. These certificates ensure that the binary contains all and only the statements from the source code, preserves their order, and maintains equivalent def-use dependencies. The certificate is represented as an integer derivable from both the source code and the binary using a concise set of derivation rules, each applied in constant time. To demonstrate the practicality of our method, we present Charon, a compiler designed to handle a subset of C expressive enough to compile FaCT, the Flexible and Constant Time cryptographic programming language.

Ken Thompson在其1984年的图灵奖演讲中表明,编译者可能被恶意地改变,将后门插入其编译的程序,并通过修改随后建立的任何编译者使这种行为永久化。Thompson的黑客被复制到真实世界的系统中,以示范为目的。提出了几项对策,以防范Thompson式的幕后门,包括众所周知的Lit Diverseule-discluding(DDC)技术,以及翻译验证和CompCert-sty 汇编等方法。然而,这些办法最终会回溯到一个基本问题:“我们如何信任用于编译工具的编译者?”在本文件中,我们引入了一种创新的方法来生成证书,以保证二元图像忠实地代表源代码。这些证书确保二进制包含来自源代码的所有声明,维护其秩序,并保持等量的自用词依赖性。证书代表一种从源代码和二进制的精确的定量,每个时间都应用一套简洁的衍生规则。为了展示我们的方法的实用性,我们制作了一个非常精确的版本,我们制作了一个快速的版本。

---

## Article 27
### Title@2025-08-16 (6): Discovering Expert-Level Nash Equilibrium Algorithms with Large Language   Models

**Title**: Discovering Expert-Level Nash Equilibrium Algorithms with Large Language   Models | Entdeckung von Experten-Nash-Equilibrium-Algorithmen mit großen Sprachmodellen | 发现具有大语言模型的专家级 Nash 平衡等级与大语言模型 [2508.11874v1](http://arxiv.org/abs/2508.11874v1)

**Authors** (3): Hanyu Li, Dongchen Li, Xiaotie Deng

Algorithm design and analysis is a cornerstone of computer science, but it confronts a major challenge. Proving an algorithm's performance guarantee across all inputs has traditionally required extensive and often error-prone human effort. While AI has shown great success in finding solutions to specific problem instances, automating the discovery of general algorithms with such provable guarantees has remained a significant barrier. This challenge stems from the difficulty of integrating the creative process of algorithm design with the rigorous process of formal analysis. To address this gap, we propose LegoNE, a framework that tightly fuses these two processes for the fundamental and notoriously difficult problem of computing approximate Nash equilibria. LegoNE automatically translates any algorithm written by a simple Python-like language into a constrained optimization problem. Solving this problem derives and proves the algorithm's approximation bound. Using LegoNE, a state-of-the-art large language model rediscovered the state-of-the-art algorithm for two-player games within hours, a feat that had taken human researchers 15 years to achieve. For three-player games, the model discovered a novel algorithm surpassing all existing human-designed ones. This work demonstrates a new human-machine collaborative paradigm for theoretical science: humans reason at a higher-abstract level, using symbols to compress the search space, and AI explores within it, achieving what neither could alone.

算法设计和分析是计算机科学的基石,但它面临一个重大挑战。 证明算法在所有投入中的性能保障历来需要大量且往往容易出错的人类努力。 虽然AI在寻找具体问题的解决方案方面表现出了巨大的成功, 但发现通用算法的自动化和这种可验证的保证仍然是一大障碍。 这一挑战源于将算法设计的创新过程与严格的正式分析过程结合起来的困难。 为了解决这一差距, 我们提议了Legone, 这个框架将这两个过程紧密结合到计算近似纳什利比里亚这一基本和臭名昭著的困难问题中。 LegoNe自动将用简单平方语言撰写的任何算法转化为一个有限的优化问题。 解决这个问题产生并证明算法的近似性。 使用最先进的大语言模型 — — 即现代的算法模型 — — 重新发现两个玩家游戏的现代算法, 这个框架花了人类研究人员15年的时间才能完成。 对于三个玩家游戏来说, 模型既不自动地将任何一种算法式的算法转换成一个高端的模型, 也可以在人类的模型中找到一个新的空间模型。

---

## Article 28
### Title@2025-08-15 (5): CoMoNM: A Cost Modeling Framework for Compute-Near-Memory Systems

**Title**: CoMoNM: A Cost Modeling Framework for Compute-Near-Memory Systems | CoMoNM: Ein Kostenmodellierungsrahmen für Compute-Near-Memory-Systeme | CONMNM: 计算近记忆系统的成本建模框架 [2508.11451v1](http://arxiv.org/abs/2508.11451v1)

**Authors** (3): Hamid Farzaneh, Asif Ali Khan, Jeronimo Castrillon

Compute-Near-Memory (CNM) systems offer a promising approach to mitigate the von Neumann bottleneck by bringing computational units closer to data. However, optimizing for these architectures remains challenging due to their unique hardware and programming models. Existing CNM compilers often rely on manual programmer annotations for offloading and optimizations. Automating these decisions by exploring the optimization space, common in CPU/GPU systems, is difficult for CNMs as constructing and navigating the transformation space is tedious and time consuming. This is particularly the case during system-level design, where evaluation requires time-consuming simulations. To address this, we present CoMoNM, a generic cost modeling framework for CNM systems for execution time estimation in milliseconds. It takes a high-level, hardware-agnostic application representation, target system specifications, and a mapping specification as input and estimates the execution time for the given application on the target CNM system. We show how CoMoNM can be seamlessly integrated into state-of-the-art CNM compilers, providing improved offloading decisions. Evaluation on established benchmarks for CNM shows estimation errors within 7.80% and 2.99%, when compared to the real UPMEM CNM system and Samsung's HBM-PIM simulator. Notably, CoMoNM delivers estimates seven orders of magnitude faster compared to the UPMEM and HBM-PIM simulators.

通过使计算单位更接近数据,计算-早期记忆(CNM)系统为减轻 von Neumann瓶颈提供了一种很有希望的方法,通过使计算单位更接近于数据来减轻 von Neumann 瓶颈。然而,由于这些结构的独特硬件和编程模式,优化这些结构仍具有挑战性。现有的全国妇女委员会编译者往往依靠人工编程说明来卸载和优化。通过探索CPU/GPU系统中常见的优化空间,使这些决策自动化起来是困难的,因为建造和导航转换空间既乏味又耗时。在系统设计阶段,评价需要花费时间的模拟。为此,我们介绍COMM(CNM)系统通用成本建模框架,用于在毫秒内进行执行估计。它需要高层次、硬件化的应用说明、目标系统规格,以及绘图规格,作为投入和估计在目标CNMFM系统上应用的执行时间。我们展示了CMoPM(NBM-M) 780 级编译员如何顺利地融入州级的编程中,提供改进的卸载后期模拟。我们展示了CMIM-M-M 和NBMSIM(M) 系统在7.M(S-M) 和SIM) 的定序内比较了2.IM(SIM) 和S-M) 和SIM(SIM) 的比对正IM) 的常规系统内,在7.99 和SIM) 和SIM(S-IM) 的比对实际的比对正M) 和SIM(SIM) 的估测算算时,在7.M) 和SM(CM) 和SBMAFM) 的比时,对基准内,对基准内,对7.-M(C-MMMMM) 的估时,对标准的比时,对标准的比。

---

## Article 29
### Title@2025-08-15 (5): Towards Efficient Hash Maps in Functional Array Languages

**Title**: Towards Efficient Hash Maps in Functional Array Languages | Auf dem Weg zu effizienten Hash-Karten in funktionalen Array-Sprachen | 努力在功能阵列语言中建立高效的散列地图 [2508.11443v1](http://arxiv.org/abs/2508.11443v1)

**Authors** (3): William Henrich Due, Martin Elsman, Troels Henriksen

We present a systematic derivation of a data-parallel implementation of two-level, static and collision-free hash maps, by giving a functional formulation of the Fredman et al. construction, and then flattening it. We discuss the challenges of providing a flexible, polymorphic, and abstract interface to hash maps in a functional array language, with particular attention paid to the problem of dynamically sized keys, which we address by associating each hash map with an arbitrary context. The algorithm is implemented in Futhark, and the achieved GPU execution performance is compared on simple benchmark problems. We find that our hash maps outperform conventional tree/search-based approaches. Furthermore, our implementation is compared against the state-of-the-art cuCollections library, which is significantly faster for hash map construction, and to a lesser degree for lookups. We explain to which extent the performance difference is due to low-level code generation limitation in the Futhark compiler, and to which extent it can be attributed to the data-parallel programming vocabulary not providing the constructs necessary to express the equivalent of the algorithms used by cuCollections. We end by reflecting to which extent the functional array language programming model could, or should, be extended to address these weaknesses.

我们通过对Fredman et al. 等人的构造进行功能化的配方,系统地推导出一个数据平行的双层、静态和无碰撞的散列地图,然后将其平化。我们讨论了以功能性阵列语言为散列地图提供一个灵活、多形态和抽象的界面的挑战,特别注意动态大小的钥匙问题,我们通过将每个散列地图与任意环境联系起来来解决这一问题。算法在Futhark得到实施,而已经实现的GPU执行性能通过简单的基准问题加以比较。我们发现,我们的散列地图超越了传统的树/搜索方法。此外,我们的执行与最先进的团形图库相比较,对于散列地图的构造速度要快得多,对外观则要低得多。我们解释性能差异在多大程度上是由于Futhark编译器对低级代码生成的限制,在多大程度上可以归因于数据单级编程词汇,而没有提供必要的结构来表示等同的常规树/搜索方法。此外,我们比较我们的执行过程比对准了最先进的立方形图库库库库库库的模型,这可以反映各种功能阵列的变式,我们最后的排列方式反映了这些变式的变式。

---

## Article 30
### Title@2025-08-15 (5): Generic Reduction-Based Interpreters (Extended Version)

**Title**: Generic Reduction-Based Interpreters (Extended Version) | Generische, reduktionsbasierte Interpreten (erweiterte Version) | 以一般裁减为基础的口译员(扩大版本) [2508.11297v1](http://arxiv.org/abs/2508.11297v1)

**Authors** (1): Casper Bach

Reduction-based interpreters are traditionally defined in terms of a one-step reduction function which systematically decomposes a term into a potential redex and context, contracts the redex, and recomposes it to construct the new term to be further reduced. While implementing such interpreters follows a systematic recipe, they often require interpreter engineers to write a substantial amount of code -- much of it boilerplate. In this paper, we apply well-known techniques from generic programming to reduce boilerplate code in reduction-based interpreters.

减少口译传统上是按一步骤削减职能来定义的,这一职能系统地将一个术语分解成一种潜在的变异和背景,将重新定义重新定义,以构建将进一步减少的新术语,在采用这种口译员时采用一种系统的方法,但往往要求口译员编写大量代码 -- -- 大部分是锅炉板。在本文中,我们采用从通用编程中流传的技巧,减少裁减口译员的锅炉代码。

---

## Article 31
### Title@2025-08-14 (4): Generating Compilers for Qubit Mapping and Routing

**Title**: Generating Compilers for Qubit Mapping and Routing | Kompiler für Qubit Mapping und Routing generieren | 正在生成Qubit 绘图和运行运行的编译器 [2508.10781v1](http://arxiv.org/abs/2508.10781v1)

**Authors** (5): Abtin Molavi, Amanda Xu, Ethan Cecchetti, Swamit Tannu, Aws Albarghouthi

Quantum computers promise to solve important problems faster than classical computers, potentially unlocking breakthroughs in materials science, chemistry, and beyond. Optimizing compilers are key to realizing this potential, as they minimize expensive resource usage and limit error rates. A critical compilation step is qubit mapping and routing (QMR), which finds mappings from circuit qubits to qubits on a target device and plans instruction execution while satisfying the device's connectivity constraints. The challenge is that the landscape of quantum architectures is incredibly diverse and fast-evolving. Given this diversity, hundreds of papers have addressed the QMR problem for different qubit hardware, connectivity constraints, and quantum error correction schemes.   We present an approach for automatically generating qubit mapping and routing compilers for arbitrary quantum architectures. Though each QMR problem is different, we identify a common core structure-device state machine-that we use to formulate an abstract QMR problem. Our formulation naturally leads to a domain-specific language, Marol, for specifying QMR problems-for example, the well-studied NISQ mapping and routing problem requires only 12 lines of Marol. We demonstrate that QMR problems, defined in Marol, can be solved with a powerful parametric solver that can be instantiated for any Marol program. We evaluate our approach through case studies of important QMR problems from prior and recent work, covering noisy and fault-tolerant quantum architectures on all major hardware platforms. Our thorough evaluation shows that generated compilers are competitive with handwritten, specialized compilers in terms of runtime and solution quality. We envision that our approach will simplify development of future quantum compilers as new quantum architectures continue to emerge.

量子计算机承诺以比古典计算机更快的速度解决重要问题,这有可能释放材料科学、化学等领域的突破。优化编译者是实现这一潜力的关键,因为他们最大限度地减少昂贵的资源使用和限制误差率。一个关键的编译步骤是qubit绘图和路由(QMR ) , 它在目标装置和计划中找到从电路qubit到qubit的映射, 在满足设备连通性限制的同时, 指令执行。 挑战在于量子结构的景观是极其多样化和快速变化的。 鉴于这种多样性, 数百篇论文已经解决了不同qubit硬件、连通性限制和量子错误校正计划等的QMR问题。 我们提出了一个自动生成qubet绘图和路径编程编辑者的方法。 尽管每个QMRMR问题都不同, 我们找到一个共同的核心结构- 代号机器,我们用来设计一个抽象的QMRR问题。 我们的配制自然导致一个特定域的量子语言, Marol , 用于说明QMR的问题, 例如, 正确研究的QQQ 的精准的Q , Marrrral 的地图和直径直径直线路路路路路路的计算方法需要我们定义的任何问题, 。我们只有任何直标的直线路路路路路路路的计算, 我们只能通过任何直路的直路的直路标, 。

---

## Article 32
### Title@2025-08-14 (4): A bargain for mergesorts -- How to prove your mergesort correct and   stable, almost for free

**Title**: A bargain for mergesorts -- How to prove your mergesort correct and   stable, almost for free | Ein Schnäppchen für Mergesorts -- Wie Sie Ihr Mergesort korrekt und stabil beweisen, fast kostenlos | 合并的讨价还价 -- -- 如何证明合并的正确和稳定,几乎是免费的 [2403.08173v3](http://arxiv.org/abs/2403.08173v3)

**Authors** (2): Cyril Cohen, Kazuhiko Sakaguchi

We present a novel characterization of stable mergesort functions using relational parametricity, and show that it implies the functional correctness of mergesort. As a result, one can prove the correctness of several variations of mergesort (e.g., top-down, bottom-up, tail-recursive, non-tail-recursive, smooth, and non-smooth mergesorts) by proving the characteristic property for each variation. Thanks to our characterization and the parametricity translation, we deduced the correctness results, including stability, of various implementations of mergesort for lists, including highly optimized ones, in the Rocq Prover (formerly the Coq Proof Assistant).

我们用相对准度来描述稳定合并的功能,并表明它意味着合并的功能正确性。 因此,我们可以通过证明每种变异的特征属性,来证明合并的几种变异(例如自上而下、自下而上、尾部-后向、尾部-后向、非尾部-后向、非尾部-后向、平稳和非间向合并的)的正确性。 由于我们的定性和对准度的翻译,我们推断出对罗克普罗维尔(前科克验证助理)的合并(包括高度优化的合并)清单的各种执行的正确性,包括稳定性。

---

